...
- 支持下载系统默认模板、下载当前方案，以及上传结构不变的 Markdown
  模板并导入为新方案。
- 长文和图文默认方案分别实时读取 `prompts/longform/default.md` 与
  `prompts/image-note/default.md`；页面方案分别保存在
  `.local-data/prompts/longform.json` 与 `.local-data/prompts/image-note.json`。
...
- 图文模式生成结构化中文卡片，支持逐页编辑、套用当前方案视觉主题、缩略图
  翻页，以及当前、选中或全部 PNG 直接下载；不生成 ZIP。
...
- 两类自定义提示词方案独立保存在本地服务端 `.local-data/prompts/`。
