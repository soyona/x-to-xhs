...
把任意来源中值得深入讨论的内容，转化为可审稿、可检查、可按步骤发布的小红书「写长文」草稿。

内容 → 小红书是一个本地运行的内容创作工作台。它不是逐句翻译器，也不是自动搬运或自动发布工具；它接收任意文本或公开网页链接，保留来源元数据，再把生成结果拆成适合小红书发布流程的独立字段。
...
- 持续关注不同平台和网站上 AI 工具、Agent、Coding 与开发者效率话题的中文创作者。
- 需要把多来源信息转化为小红书长文，但不想反复手工改写和拆分发布内容的个人作者。
- 希望保留人工判断和最终编辑权，同时用确定性规则降低格式错误的知识型创作者。
- 重视 API Key 的本地保存，并愿意在生成内容时将素材发送给已配置且实际尝试的第三方模型服务处理的用户。

### 你可能正在面对的问题

- 原始素材信息结构各异，直接搬运后不符合小红书的阅读节奏。
...
内容 → 小红书把一份原始素材变成一条可控的创作链路：

```text
任意文本、公开网页 URL 或自主编写内容
        ↓
按小红书长文规范进行二度创作
        ↓
按当前提示词方案生成
...
- 支持粘贴来自社交平台、网页、文档、个人笔记等任意来源的文本。
- 支持可公开访问的 `http(s)` 网页 URL，并自动提取可读正文。
- 支持自主编写内容；无链接纯文本可直接生成。
- 粘贴内容时可在末尾附上来源链接，系统会自动保留来源元数据。
- X/Twitter 帖子 URL 继续通过公开 oEmbed 接口进行专用解析。
...
- 标签明确区分“基于正文生成”和未来可接入的趋势数据模式；没有可信
  趋势数据时不宣称实时热门。
- 保留固定的摘要与推荐标签结构。
- 生成后分别复制长文标题、长文正文、正文描述和标签，不提供“一次复制全文”。
...
1. **01 内容创作**：输入完整帖子文本或单条帖子 URL。
2. **02 小红书笔记**：分别复制四个内容区块，并按需重新生成对应部分。
...
- 长文标题：从 3 个候选中选择；不满意时可重新生成 3 个。
- 长文正文：可主动生成新正文版本，也可直接复制，或导出仅包含正文的 Markdown 文件后，
   在小红书标题与正文编辑页选择“从文件导入”。导入会覆盖编辑器内已有正文。
- 正文描述：人工审核不通过时可重新生成并确认新版本。
- 标签：不满意时可重新生成，且界面明确当前未使用实时趋势数据。
...
- 第三方素材默认生成独立解读，不逐句翻译或复刻原文结构；有来源元数据时，复制与 Markdown 导出会附加来源说明。
- 自主编写或无链接文本不强制要求来源链接。
...
- 健康检查和设置接口不会返回 Key。
- 配置弹窗不会回显已保存的 Key。
- 本项目的本地服务不会主动把输入内容、提示词或完整模型响应写入日志。
- 解析 X URL 时，帖子 URL 会发送给 X 的公开 oEmbed 接口。
- 生成或重新生成时，输入内容、当前提示词和相关草稿会发送给自动降级链路中实际尝试的第三方模型服务。
