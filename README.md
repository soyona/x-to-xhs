...
生成稿内部保留`#`、`##`、`-`和`1.`结构标记，用于标题与列表语义及确定性检查。复制正文时同时写入`text/html`富文本和去除井号的`text/plain`兜底：目标编辑器接受富文本时可获得标题/列表结构，只接受纯文本时也不会出现字面井号。清理逻辑仍会移除加粗、代码围栏、分隔线和 Markdown 表格等非目标语法。
...
- 生成含 2 个 Emoji 的 18–20 字标题；正文按原文信息量自然展开，上限 10000 字。
- 可配置目标读者、内容目标、语气、深度、术语、案例、Emoji 和行动建议。
- 可补充作者表达身份、禁用表达及 500 字以内的高级创作指令。
- 个性化偏好不会覆盖固定输出结构、事实边界或平台硬性规范。
- 拆分长文标题、长文正文、发布标题、正文描述和标签。
- 长文标题和发布标题分别生成 3 个候选，支持选择及重新生成；发布标题
  独立于长文标题并执行 20 字上限。
- 正文、正文描述和标签支持主动重新生成；采用新正文后，发布标题、
  正文描述和标签会标记为基于旧正文，等待重新生成或人工确认。
...
2. **02 小红书长文**：按发布步骤复制内容，并在对应部分查看检查结果和执行修复。
...
1. 从 3 个合格候选中选择长文标题；不满意时可重新生成 3 个。
2. 输入长文正文；可主动生成新正文版本，也可直接复制，或导出仅包含正文的 Markdown 文件后，
   在小红书标题与正文编辑页选择“从文件导入”。导入会覆盖编辑器内已有正文。
3. 点击「一键排版」。
4. 从 3 个不超过 20 字的独立发布标题中选择；不满意时可重新生成。
5. 输入正文描述；人工审核不通过时可重新生成并确认新版本。
6. 输入标签；不满意时可重新生成，且界面明确当前未使用实时趋势数据。
7. 检查并发布。
